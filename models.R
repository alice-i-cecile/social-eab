...
eab_factory <- function (r=1, K=1, allee_threshold=0, transmission_rate=0,   fatality_rate=0,  sigma=0, transport_cost=0, local_cost=0, norm_strength=0, concern_level=0, dispersal_constant=0){
  ...
  d_local_func <- function (local, infected){
    ...
    #d_local <- sigma*local*(1-local)*(transport_cost-local_cost+norm_strength*(2*local-1)+concern_level*infected)
    
    d_local <- sigma*local*(1-local)*local_payoff
